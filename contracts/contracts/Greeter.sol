    ...
    Ownable
{
    ...
    function safeMint(
        ...
        require(msg.value >= 0.001 ether, "Not enough ether for mint");
        require(balanceOf(msg.sender) == 0, "Already minted an NFT");

        string memory uri = string(abi.encode(name, img));
    ...
    function getTokenURI(uint256 tokenId) public view returns (string memory) {
        ...
        (string memory name, string memory img) = abi.decode(
            bytes(tokenData),
            (string, string)
        );
    ...
    function getAccounts() public view returns (Account[] memory) {
        ...
        Account[] memory accounts = new Account[](totalSupply);
        for (uint256 i = 0; i < totalSupply; i++) {
            address tokenHolderAddress = ownerOf(i);
            string memory tokenDataEncoded = tokenURI(i);
            (string memory name, string memory img) = abi.decode(
                bytes(tokenDataEncoded),
                (string, string)
            );
            accounts[i] = Account(tokenHolderAddress, name, img);
        }
        return accounts;
    }
}
